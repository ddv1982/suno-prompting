...
function ShuffleBtn({
  ...
  current: GeneratingAction;
  onClick: () => void;
}): React.ReactElement {
  return (
    <Button variant="outline" size="sm" onClick={onClick} autoDisable className="font-bold">
      <Shuffle className={cn('w-3.5 h-3.5', current === action && 'animate-spin')} />
      {label}
    </Button>
  );
}
...
function FieldButtons({
  ...
  onRemixStyleTags,
  onRemixRecording,
}: FieldButtonsProps): React.ReactElement {
  return (
    <>
      {fields.hasGenre && (
        <ShuffleBtn
      ...
      {fields.hasInstruments && (
        <ShuffleBtn
          label="INSTRUMENTS"
          action="remixInstruments"
          current={generatingAction}
