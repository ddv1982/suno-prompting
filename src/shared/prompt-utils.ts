...
export function isStructuredPrompt(text: string): boolean {
  ...
  const hasNonMaxFields = /^(Genre|BPM|Mood|Instruments):\s*\S/im.test(bodyText);
  ...
  const hasMaxStyleFields = /^(genre|bpm|instruments|style tags|recording):\s*"/im.test(bodyText);
...
export function detectRemixableFields(text: string): DetectedFields {
  ...
  return {
    ...
    hasGenre: /^(Genre|genre):\s*["']?\S/im.test(bodyText),
    // Mood in standard format (Capitalized) or MAX format (lowercase quoted)
    hasMood: /^(Mood|mood):\s*["']?\S/im.test(bodyText),
    // Instruments in both formats
    hasInstruments: /^(Instruments|instruments):\s*["']?\S/im.test(bodyText),
    // Style tags only in MAX format (lowercase, quoted)
    hasStyleTags: /^style tags:\s*"/im.test(bodyText),
    // Recording only in MAX format (lowercase, quoted)
    hasRecording: /^recording:\s*"/im.test(bodyText),
  };
}
