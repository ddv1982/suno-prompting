...
describe('StorageManager', () => {
  async function withStorage<T>(
    run: (context: { testDir: string; storage: StorageManager }) => Promise<T>
  ) {
    const testDir = join(
      tmpdir(),
      `suno-storage-${Date.now().toString()}-${Math.random().toString(36).slice(2)}`
    );
    const storage = new StorageManager(testDir);
    await storage.initialize();

    try {
      return await run({ testDir, storage });
    } finally {
  ...
  test('saveConfig does not overwrite an unreadable config file', async () => {
    await withStorage(async ({ testDir, storage }) => {
