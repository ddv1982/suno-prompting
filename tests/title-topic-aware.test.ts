...
describe('Topic-Aware Title Generation', () => {
  describe('generateDeterministicTitle with description', () => {
    test('generates topic-aware title with midnight theme', () => {
      ...
      for (let i = 0; i < 10; i++) {
        const title = generateDeterministicTitle('pop', 'melancholic', () => i / 10, 'midnight memories');
        
        // Check if title includes midnight-related words
        if (title.includes('Midnight') || title.includes('Night') || 
            title.includes('Memory') || title.includes('Dream')) {
    ...
    test('generates topic-aware title with ocean theme', () => {
      ...
      let foundTopicAware = false;
      for (let i = 0; i < 10; i++) {
        const title = generateDeterministicTitle('ambient', 'calm', () => i / 10, 'peaceful ocean waves');
        
        // Check if title includes ocean-related words
        if (title.includes('Ocean') || title.includes('Waves') || 
            title.includes('Water') || title.includes('River')) {
          foundTopicAware = true;
          break;
        }
      }
    ...
    test('prioritizes topic keywords over generic mood filtering', () => {
      ...
      const seededRng = () => 0.5;
      const title = generateDeterministicTitle('ambient', 'calm', seededRng, 'thunderstorm approaching');
