...
describe('canRefineFullPrompt', () => {
  ...
  describe('feedback-only refinement', () => {
    ...
    test('returns true with long feedback text', () => {
      expect(canRefineFullPrompt({
        feedbackText: 'Make the lyrics more poetic and add metaphors about the ocean and waves crashing on the shore. The chorus should be more memorable and catchy.',
        styleChanges: undefined,
        lyricsMode: true,
      })).toBe(true);
    });
