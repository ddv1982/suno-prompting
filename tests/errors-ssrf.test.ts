...
describe('SSRF Prevention: validateOllamaEndpoint', () => {
  ...
  describe('IPv6 localhost variations (bypass attempts)', () => {
    ...
    bypassAttempts.forEach((endpoint) => {
      test(`should block IPv6 bypass attempt: ${endpoint}`, () => {
        expect(() => validateOllamaEndpoint(endpoint)).toThrow(InvalidOllamaEndpointError);
      });
    });
  ...
  describe('port validation', () => {
    ...
    test('should reject privileged ports (< 1024)', () => {
      expect(() => validateOllamaEndpoint('http://127.0.0.1:80')).toThrow(ValidationError);
      expect(() => validateOllamaEndpoint('http://127.0.0.1:22')).toThrow(ValidationError);
      expect(() => validateOllamaEndpoint('http://127.0.0.1:443')).toThrow(ValidationError);
    });

    test('should reject URLs without explicit port (uses default privileged port)', () => {
      // http defaults to port 80 (privileged)
      expect(() => validateOllamaEndpoint('http://127.0.0.1')).toThrow(ValidationError);
      expect(() => validateOllamaEndpoint('http://localhost')).toThrow(ValidationError);
      expect(() => validateOllamaEndpoint('http://[::1]')).toThrow(ValidationError);
    });
